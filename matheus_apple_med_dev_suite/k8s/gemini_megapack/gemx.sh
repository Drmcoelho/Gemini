...
GEMX_PLUGINS="${GEMX_PLUGINS:-./plugins.d}"
...
# ---------- Helpers portáteis ----------
plugin_list(){ ls -1 "$GEMX_PLUGINS"/*.sh 2>/dev/null | xargs -n1 basename 2>/dev/null | sed 's/\.sh$//' || true; }
plugin_run(){ local n="${1:-}"; shift || true; [ -n "$n" ] || { warn "plugin nome vazio"; return 2; }; local p="$GEMX_PLUGINS/$n.sh"; [ -f "$p" ] || { warn "plugin não encontrado: $n"; return 3; }; "$p" "$@"; }
