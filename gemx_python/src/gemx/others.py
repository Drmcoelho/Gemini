...
def _show_extensions_menu(extensions: List[Dict[str, Any]]):
    """Mostra o menu para a categoria 'Extensões'."""
    while True:
        table = Table(title="[bold]Extensões[/bold]", show_header=True, header_style="bold magenta")
        table.add_column("#", style="dim")
        table.add_column("ID")
        table.add_column("Descrição")
        
        for i, item in enumerate(extensions):
            table.add_row(str(i + 1), item.get("id", "N/A"), item.get("description", "N/A"))
        
        console.print(table)
        console.print("[bold]Digite o número da extensão para instalar, ou 'v' para voltar.[/bold]")
        ...
        try:
            ...
            if 0 <= index < len(extensions):
                ext_id = extensions[index].get("id")
                if ext_id:
                    console.print(f"[cyan]Instalando extensão:[/cyan] [bold]{ext_id}[/bold]")
                    _run_shell_command(f"gh extension install {ext_id}")
                else:
                    console.print("[red]ID da extensão não encontrado.[/red]")
            else:
                console.print("[red]Número inválido.[/red]")
        except ValueError:
            console.print("[red]Entrada inválida.[/red]")
...
def _show_plugins_menu(plugins: List[Dict[str, Any]]):
    """Mostra o menu para a categoria 'Plugins'."""
    while True:
        table = Table(title="[bold]Plugins[/bold]", show_header=True, header_style="bold magenta")
        table.add_column("#", style="dim")
        table.add_column("Key")
        table.add_column("Descrição")
        table.add_column("Estado Atual")
        
        current_config_plugins = config.get_config_file_content().get("plugins", {})

        for i, item in enumerate(plugins):
            key = item.get("key", "N/A")
            current_state = current_config_plugins.get(key, item.get("default", False))
            table.add_row(str(i + 1), key, item.get("description", "N/A"), str(current_state))
        
        console.print(table)
        console.print("[bold]Digite o número do plugin para alternar o estado, ou 'v' para voltar.[/bold]")
        ...
        try:
            ...
            if 0 <= index < len(plugins):
                plugin_key = plugins[index].get("key")
                if plugin_key:
                    current_state = current_config_plugins.get(plugin_key, plugins[index].get("default", False))
                    new_state = not current_state
                    console.print(f"[cyan]Alternando plugin '{plugin_key}' para:[/cyan] [bold]{new_state}[/bold]")
                    config.update_config_value(f"plugins.{plugin_key}", new_state)
                    console.print("[green]✓ Estado atualizado.[/green]")
                else:
                    console.print("[red]Chave do plugin não encontrada.[/red]")
            else:
                console.print("[red]Número inválido.[/red]")
        except ValueError:
            console.print("[red]Entrada inválida.[/red]")
...
def _show_resources_menu(resources: List[Dict[str, Any]]):
    """Mostra o menu para a categoria 'Recursos'."""
    while True:
        table = Table(title="[bold]Recursos[/bold]", show_header=True, header_style="bold magenta")
        table.add_column("#", style="dim")
        table.add_column("Descrição")
        table.add_column("URL")
        
        for i, item in enumerate(resources):
            table.add_row(str(i + 1), item.get("description", "N/A"), item.get("url", "N/A"))
        
        console.print(table)
        console.print("[bold]Digite o número do recurso para ver a URL, ou 'v' para voltar.[/bold]")
        ...
        try:
            ...
            if 0 <= index < len(resources):
                resource_url = resources[index].get("url")
                if resource_url:
                    console.print(f"[cyan]URL do recurso:[/cyan] [bold]{resource_url}[/bold]")
                    # Em um ambiente real, você poderia usar `webbrowser.open(resource_url)`
                else:
                    console.print("[red]URL do recurso não encontrada.[/red]")
            else:
                console.print("[red]Número inválido.[/red]")
        except ValueError:
            console.print("[red]Entrada inválida.[/red]")
